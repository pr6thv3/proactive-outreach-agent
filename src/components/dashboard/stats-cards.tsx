...
export function StatsCards({ stats }: Props) {
  const cards = [
    ...
    { label: 'Avg Score', value: parseFloat(stats?.leads.avgLeadScore || '0').toFixed(0), sub: `max: ${stats?.leads.maxLeadScore || 0}`, icon: Target, bg: 'bg-amber-500/10', tx: 'text-amber-400', br: 'border-amber-500/20' },
    ...
    { label: 'Memory', value: stats?.memory?.totalEntries ?? 0, sub: `${stats?.memory?.categories?.length ?? 0} categories`, icon: Brain, bg: 'bg-violet-500/10', tx: 'text-violet-400', br: 'border-violet-500/20' },
  ];
