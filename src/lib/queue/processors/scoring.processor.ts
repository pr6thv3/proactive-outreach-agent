...
export async function processScoringJob(data: ScoringJobData) {
  ...
  const buildContext = (orchestrator as unknown as { buildContext: (leadId: string, campaignId?: string, organizationId?: string, traceId?: string) => Promise<unknown> }).buildContext;
  const context = await buildContext.call(orchestrator, data.leadId, data.campaignId, data.organizationId, data.traceId);
