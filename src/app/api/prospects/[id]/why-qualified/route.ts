...
//   icpMatchBreakdown: { firmographicScore: number, technographicScore: number, intentScore: number, mxScore: number, totalScore: number },
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const lead = await db.lead.findFirst({
      where: process.env.AUTH_DEV_BYPASS === 'true'
        ? { id }
    ...
    if (!lead) {
      return fail('Prospect not found', 404, 'not_found', traceId);
    }
